...
interface SlickProductCardProps {
  name: string;
  image: string;
  desc: string;
  href?: string;
...
export const SlickProductCard = ({
  ...
  hasArrow,
  openDialog,
}: SlickProductCardProps) => {
  return (
    <div
      className="relative -mt-4 flex min-h-[90vh] w-full flex-col items-center justify-center gap-8 bg-[#FEE600]/50 bg-cover bg-fixed bg-center bg-no-repeat p-8 px-4 py-8 bg-blend-multiply"
      style={{
        ...
        <div className="mt-20 flex flex-col items-center justify-center gap-4">
          {name === "Vivian AI Chat Bot" ? (
            <div
              ...
              onClick={openDialog}
            >
              <BigTitle title={name} /> <ArrowUpRight className="size-12" />
            </div>
          ) : (
            <a
              href={href}
